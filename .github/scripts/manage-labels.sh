...
if [ `gsutil ls gs://distribution-prestabulles |grep $DISTRIBUTION_APP |wc -l` -eq 1 ]
then
  echo "Current application $DISTRIBUTION_APP has already been deployed, let's keep checking..."
  
  if [ `gsutil ls gs://distribution-prestabulles/$DISTRIBUTION_APP/|grep $BULLE |wc -l` -eq 1 ]
  then
    echo "Current $BULLE prestabulle has already been defined as well, let's keep checking..."

    gsutil -q cp gs://distribution-prestabulles/$DISTRIBUTION_APP/$BULLE .
    ...
    then
      ...
      for i in `gsutil ls gs://distribution-prestabulles/$DISTRIBUTION_APP/ |awk -F'/' '{ print $NF}'`
      do
        echo $i
      done 
  ...
  gsutil -q cp $BULLE  gs://distribution-prestabulles/$DISTRIBUTION_APP/
